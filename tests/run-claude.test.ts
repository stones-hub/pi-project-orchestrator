...
function writeFakeClaude(dir: string, outputJson: unknown, exitCode = 0, stderrText = "") {
...
describe("scripts/run-claude", () => {
  ...
  it("builds the real investigate-mode argv shape and reports success", () => {
    const script = writeFakeClaude(dir, {
      session_id: "s1",
      is_error: false,
      result: "ok",
  ...
  it("explicitly disables all MCP tools in both investigate and review read-only modes", () => {
    for (const mode of ["investigate", "review"]) {
      ...
      const script = writeFakeClaude(localDir, { session_id: "mcp1", is_error: false, result: "ok", usage: {} });
      const { exitCode } = run([
        "--mode",
        mode,
        "--prompt",
  ...
  it("builds the code-mode argv with allowedTools and resume", () => {
    const script = writeFakeClaude(dir, { session_id: "s2", is_error: false, result: "done", usage: {} });
    const { exitCode, output } = run([
      "--mode",
      "code",
      "--prompt",
  ...
  it("binds the spawned process cwd to --workspace", () => {
    ...
    const script = writeFakeClaude(dir, { session_id: "s3", is_error: false, result: "ok", usage: {} });
    const { exitCode } = run([
      "--mode",
      "investigate",
      "--prompt",
  ...
  it("persists stdout and stderr to separate evidence files and returns both paths without leaking raw content in the summary", () => {
    ...
    const script = writeFakeClaude(dir, { session_id: "s6", is_error: false, result: "ok", usage: {} }, 0, secret);
  ...
  it("does not report success when the response JSON has a session_id but no terminal result/is_error shape", () => {
    const script = writeFakeClaude(dir, { session_id: "s5", usage: {} });
    const { exitCode, output } = run([
      "--mode",
      "investigate",
      "--prompt",
  ...
  it("reports failure when is_error is true even with exit code 0", () => {
    const script = writeFakeClaude(dir, { session_id: "s4", is_error: true, result: "denied", usage: {} });
    const { exitCode, output } = run([
      "--mode",
      "investigate",
      "--prompt",
